  ...
  return (
    <main className={styles.page} id="conteudo">
      ...
      <article className={styles.sheet}>
        ...
        <section className={styles.block}><h2><span>02</span> {projectLabels.proofBodiesHeading} corpos de prova</h2><HorizontalScrollRegion className={styles.scrollRegion} label="Tabela dos corpos de prova; role horizontalmente para ver todas as colunas"><table className={styles.table}><thead><tr><th>Placa</th><th>Sistema</th><th>Insumos / lote</th><th>Observação depois da cura</th><th>Decisão</th></tr></thead><tbody>{proofBodies.map((proof) => <tr key={proof.code}><th scope="row">{proof.code}</th><td>{proof.title}</td><td><Blank /></td><td><Blank /></td><td>□ reprovar<br />□ repetir<br />□ aprovar</td></tr>)}</tbody></table></HorizontalScrollRegion></section>

        <section className={styles.block}><h2><span>03</span> Acabamento e uso</h2><HorizontalScrollRegion className={styles.scrollRegion} label="Checklist de acabamento e uso; role horizontalmente para ver todos os campos"><div className={styles.checks}>{useTests.map((test) => <div key={test}><strong>{test}</strong><span>□ falhou</span><span>□ revisar</span><span>□ passou</span><Blank /></div>)}</div></HorizontalScrollRegion></section>

        <section className={styles.block}><h2><span>04</span> Estrutura montada</h2><HorizontalScrollRegion className={styles.scrollRegion} label="Checklist da estrutura; role horizontalmente para ver todos os campos"><div className={styles.checks}>{structureTests.map((test) => <div key={test}><strong>{test}</strong><span>□ falhou</span><span>□ revisar</span><span>□ passou</span><Blank /></div>)}</div></HorizontalScrollRegion><label className={styles.line}>Carga aplicada, duração e método <Blank wide /></label></section>
        ...
        <section className={styles.block}><h2><span>06</span> Fechamento do Portão {edition.prototypeNumber}</h2><HorizontalScrollRegion className={styles.scrollRegion} label={`Matriz do Portão ${edition.prototypeNumber}; role horizontalmente para ver todos os campos`}><div className={styles.gate}>{prototypeGate.map((item) => <div key={item.code}><strong>{item.code} {item.area}</strong><span>□ em aberto</span><span>□ em teste</span><span>□ aprovado</span><label>Data <Blank /></label><label>Rubrica <Blank /></label></div>)}</div></HorizontalScrollRegion><div className={styles.decision}><p>Decisão do ciclo</p><span>□ reprovar o sistema</span><span>□ retrabalhar e repetir evidências</span><span>□ liberar ficha técnica para a peça {edition.firstPiece}</span></div></section>
