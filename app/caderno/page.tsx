  ...
  return (
    <main className="notebook-page" id="conteudo">
      ...
      <section className="notebook-section prototypes" id="testes">
        <div className="notebook-section-title"><span>03</span><div><p className="micro-label">Matriz de protótipos</p><h2>{projectLabels.proofBodiesHeading} placas<br />antes da mesa.</h2></div></div>
        <div className="prototype-grid">{proofBodies.map((test) => <article key={test.code}><strong>{test.code}</strong><FlaskConical /><h3>{test.title}</h3><p>{test.detail}</p><span>Placa {productLabels.proofPlate}</span></article>)}</div>
